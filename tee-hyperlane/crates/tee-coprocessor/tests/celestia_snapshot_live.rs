...
use tee_node::origins::celestia::{celestia_root, CelestiaStore};
use tee_node::state_proofs::verify_celestia_merkle_tree;
...
async fn a_snapshot_proof_verifies_under_the_height_it_was_read_at() {
    ...
    let hook_id: [u8; 32] = hex::decode(HOOK_ID.trim_start_matches("0x"))
        .unwrap()
        .try_into()
        .unwrap();
    ...
    let app_hash = |h: u64| {
        ...
        async move {
            ...
            celestia_root(&CelestiaStore { trusted: block })
                .unwrap()
                .state_root
                .0
        }
    ...
    let (bytes_earlier, proof_earlier) = reader
        .merkle_tree_hook_proof(hook_id, earlier)
        .await
        .unwrap();
    ...
    verify_celestia_merkle_tree(root_at, hook_id, &bytes_at, &proof_at)
        .expect("the head proof must verify under the head's app hash");
    verify_celestia_merkle_tree(root_earlier, hook_id, &bytes_earlier, &proof_earlier)
    ...
    assert!(
        verify_celestia_merkle_tree(root_at, hook_id, &bytes_earlier, &proof_earlier).is_err(),
        "a proof read at {earlier} must not verify under the app hash at {at}"
    );
    assert!(
        verify_celestia_merkle_tree(root_earlier, hook_id, &bytes_at, &proof_at).is_err(),
